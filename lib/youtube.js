...
async function getPoToken() {
	...
	if (process.env.YOUTUBE_PO_TOKEN) {
		return { poToken: process.env.YOUTUBE_PO_TOKEN, visitorData: process.env.YOUTUBE_VISITOR_DATA };
	}
...
async function getVideoInfo(videoUrl) {
	...
	return new Promise((resolve, reject) => {
		let extractorArgs = 'youtube:player_client=web;player_skip=android,ios';
		
		if (tokenResult && tokenResult.poToken) {
			extractorArgs += `;po_token=web+${tokenResult.poToken}`;
			if (tokenResult.visitorData) {
				extractorArgs += `;visitor_data=${tokenResult.visitorData}`;
			}
			console.log('使用 PO Token 與 Visitor Data 進行驗證...');
		}
